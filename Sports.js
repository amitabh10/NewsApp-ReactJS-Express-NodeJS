...
class Sports extends React.Component {
	...
	componentDidMount(){
		...
		var url = this.props.guardian? "http://localhost:3001/stories?src=guardian&cat=sport" : "http://localhost:3001/stories?src=nytimes&cat=sport"
		fetch(url)
		.then(res => res.json())
		.then((result) => {
	...
	componentDidUpdate(prevProps){
		...
		var url = this.props.guardian ? "http://localhost:3001/stories?src=guardian&cat=Sports" : "http://localhost:3001/stories?src=nytimes&cat=sport"
		fetch(url)
		.then(res => res.json())
		.then((result) => {
			result = result.map( article => <ArticleCard key={article.id} data={article} onArticleClick ={this.props.onArticleClick}/>)
